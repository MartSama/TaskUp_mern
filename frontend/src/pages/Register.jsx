...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setAlert({})
        try {
            const { data } = await axios.post('http://localhost:4000/api/users', { name, password, email })
            console.log(data)
        } catch (error) {
            console.log(error)
        }
        ...
        <>
            ...
            <form action="" className='my-10 bg-gray-900 text-white shadow rounded-lg py-5 px-10' onSubmit={handleSubmit}>
                <div className='my-5'>
                    ...
                    <input className='w-full bg-gray-600 text-sky-500 placeholder-sky-500 py-2 px-2 rounded-lg shadow shadow-gray-400' type="text" id='name' value={name} onChange={e => setName(e.target.value)} placeholder='Vania Lopez...' />
                </div>
                <div className='my-5'>
                ...
                <div className='my-5'>
                    ...
                    <input className='w-full bg-gray-600 text-sky-500 placeholder-sky-500 py-2 px-2 rounded-lg shadow shadow-gray-400' type="password" id='password' value={password} onChange={e => setPassword(e.target.value)} placeholder='Enter a pass > 6' />
                </div>
                <div className='my-5'>
                    <label className='uppercase text-gray-300 block text-xl mb-3 font-bold' htmlFor="password2">Repeat password: </label>

                    <input className='w-full bg-gray-600 text-sky-500 placeholder-sky-500 py-2 px-2 rounded-lg shadow shadow-gray-400' type="password" id='password2' placeholder='Confirm your pass' value={repeatPassword} onChange={(e) => setRepeatPassword(e.target.value)} />
                </div>
